...
- `vps-sync-repo` syncs gitignored `.claude/` content from the laptop but **skips `.claude/worktrees/` by default** to keep payloads small (worktrees with `node_modules` can push a single repo past 5 GB). If you want worktrees on the VPS, run with `VPS_SYNC_INCLUDE_WORKTREES=1 vps-sync-repo` (or export it in the shell first). On a 40 GB cx23, watch `df -h` if you opt in for multiple repos — easy to fill the disk.
